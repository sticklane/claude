...
fn only_note(root: &Path) -> String {
    ...
    assert_eq!(files.len(), 1, "expected exactly one note file, got {files:?}");
...
fn notes_add_writes_frontmatter_and_body() {
    ...
    assert!(frontmatter_field(&note, "id").is_some(), "missing id: {note}");
    assert_eq!(frontmatter_field(&note, "anchor_path").as_deref(), Some("a.target"));
    ...
    assert!(frontmatter_field(&note, "created").is_some(), "missing created");
}
...
fn notes_add_records_kind() {
    ...
    let out = ctx(root, &["notes", "add", "target", "b", "--kind", "invariant"]);
    assert!(out.status.success(), "add failed: {}", stderr(&out));
    assert_eq!(frontmatter_field(&only_note(root), "kind").as_deref(), Some("invariant"));
}
...
fn notes_add_author_from_ctx_author_env() {
    ...
    let out = ctx_env(root, &["notes", "add", "target", "b"], "CTX_AUTHOR", "alice");
    assert!(out.status.success(), "add failed: {}", stderr(&out));
    assert_eq!(frontmatter_field(&only_note(root), "author").as_deref(), Some("alice"));
}
...
fn notes_add_author_unknown_in_no_vcs_fixture() {
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_ctx"))
        ...
        .unwrap();
    assert!(out.status.success(), "add failed: {}", stderr(&out));
    assert_eq!(frontmatter_field(&only_note(root), "author").as_deref(), Some("unknown"));
}
...
fn notes_body_sources_positional_file_and_stdin() {
    ...
    // positional
    assert!(ctx(root, &["notes", "add", "target", "positional body"]).status.success());
    // --file
    write(root, "body.md", "file-sourced body\n");
    assert!(ctx(root, &["notes", "add", "target", "--file", "body.md"]).status.success());
    // stdin via --file -
    assert!(
        ctx_stdin(root, &["notes", "add", "target", "--file", "-"], "stdin-sourced body")
            .status
            .success()
    );
    ...
    let bodies: Vec<String> = note_files(root)
        ...
        .collect();
    assert!(bodies.iter().any(|b| b.contains("positional body")), "positional");
    assert!(bodies.iter().any(|b| b.contains("file-sourced body")), "file");
    assert!(bodies.iter().any(|b| b.contains("stdin-sourced body")), "stdin");
}
...
fn notes_freshness_flips_stale_on_body_edit() {
    ...
    assert!(ctx(root, &["notes", "add", "target", "note body"]).status.success());

    let fresh = ctx(root, &["notes", "target"]);
    assert!(fresh.status.success(), "show failed: {}", stderr(&fresh));
    assert!(stdout(&fresh).contains("fresh"), "fresh before edit: {}", stdout(&fresh));
    ...
    assert!(stdout(&stale).contains("stale"), "stale after edit: {}", stdout(&stale));
}
...
fn notes_list_filters_by_kind_stale_and_file() {
    ...
    assert!(ctx(root, &["notes", "add", "target", "n1", "--kind", "gotcha"]).status.success());
    assert!(ctx(root, &["notes", "add", "other", "n2", "--kind", "todo"]).status.success());

    // --kind gotcha returns only the target note.
    let by_kind = ctx(root, &["notes", "list", "--kind", "gotcha"]);
    assert!(by_kind.status.success());
    assert!(stdout(&by_kind).contains("a.target"), "kind filter keeps target");
    assert!(!stdout(&by_kind).contains("b.other"), "kind filter drops other");

    // --file b.py returns only the other note.
    let by_file = ctx(root, &["notes", "list", "--file", "b.py"]);
    assert!(by_file.status.success());
    assert!(stdout(&by_file).contains("b.other"), "file filter keeps other");
    assert!(!stdout(&by_file).contains("a.target"), "file filter drops target");
    ...
    assert!(stale.status.success());
    assert!(stdout(&stale).contains("a.target"), "stale filter keeps stale target");
    assert!(!stdout(&stale).contains("b.other"), "stale filter drops fresh other");
}
...
fn notes_corrupted_frontmatter_is_skipped_with_diagnostic() {
    ...
    assert!(ctx(root, &["notes", "add", "target", "good note"]).status.success());
    // A second note file with unparseable frontmatter.
    write(root, ".context/notes/broken.md", "no frontmatter here at all\n");
    ...
    let diag = stderr(&out);
    assert!(diag.contains("broken.md"), "diagnostic names the bad file: {diag}");
...
fn notes_merge_conflict_only_on_the_note_file() {
    ...
    sleep(PAST);
    init(root);
    assert!(ctx(root, &["notes", "add", "target", "original line"]).status.success());

    let note = note_files(root);
    assert_eq!(note.len(), 1);
    let note_rel = note[0].strip_prefix(root).unwrap().to_string_lossy().to_string();
    ...
    assert!(git(root, &["commit", "-aqm", "other edit"]).status.success());

    // Divergent edit on the first branch.
    let default_branch = String::from_utf8(
        git(root, &["rev-parse", "--abbrev-ref", "HEAD"]).stdout,
    )
    .unwrap();
    ...
    assert_eq!(conflicted, vec![note_rel.as_str()], "only the note file conflicts");
}
...
fn c10_markers_all_surfaces_show_fresh_and_stale_note_count() {
    ...
    assert!(ctx(root, &["notes", "add", "target", "fresh note"]).status.success());
...
fn notes_deletion_freshness_purges_symbol_and_stales_note() {
    ...
    assert!(stdout(&before).contains("target"), "target present before delete");
    ...
    assert!(after.status.success(), "tree after delete: {}", stderr(&after));
    assert!(!stdout(&after).contains("def target"), "symbol purged: {}", stdout(&after));
    assert_eq!(ctx(root, &["sig", "target"]).status.code(), Some(1), "sig not found after purge");

    // The note's derived freshness now reads stale (anchor no longer resolves).
    let listed = ctx(root, &["notes", "list", "--stale"]);
    assert!(listed.status.success());
    assert!(stdout(&listed).contains("a.target"), "note reads stale: {}", stdout(&listed));
}
