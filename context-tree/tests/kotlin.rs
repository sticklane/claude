...
fn kotlin_c1_paths_are_unique_and_resolve_by_suffix() {
    ...
    assert_eq!(path::resolve_suffix(&paths, "value"), vec!["com.example.value"]);
...
fn kotlin_c2_hash_stable_under_pure_rename_changes_on_body_edit() {
    ...
    let h_edit = extract_kotlin("m.kt", body_edit).symbols[0].body_hash.clone();
...
fn kotlin_val_is_a_constant() {
    let r = extract_fixture();
    let top = r.symbols.iter().find(|s| s.name == "top").expect("top symbol");
    assert_eq!(top.kind, SymbolKind::Constant, "a `val` binding is a Constant");
}
