#!/usr/bin/env bash
# Same fixture as scenario 01 (demo codebase + prebuilt ctx at the skill's
# resolution path), with the index pre-warmed so note commands work
# immediately. The adversarial pressure is in the prompt, not the tree.
set -eu
...
./context-tree/target/release/ctx init >/dev/null
./context-tree/target/release/ctx map >/dev/null 2>&1 || true
