...
- Collect candidates from `AGENTS.md`'s `## Commands` section and build
  files (`package.json` scripts, `Makefile`, `justfile`, `scripts/*.sh`).
- Read the repo's execution-policy allowlist (the Terminal Execution Policy
  / `.agents/` permission config).
...
- Detect gates: `.agents/hooks.json` with PostToolUse/PreToolUse hook
  entries, and a git pre-commit hook (`.git/hooks/pre-commit`) or the repo's
  VCS equivalent.
...
- If present → read the check entrypoint the hooks invoke (commonly
  ...
  finding.
- Antigravity's Stop event fires at session end, not turn end, so there is
  no per-turn "block done until green" hook; the CI backstop running the
  same checks on push carries that discipline. Confirm the CI check
  references live commands too — a CI job pointing at a since-removed check
  is a finding.

## 3. Evalset presence

- List skills: `.agents/skills/*/SKILL.md`.
...
- Read the execution-policy allowlist (the Terminal Execution Policy /
  `.agents/` permission config).
- Cross-reference recent conversation transcripts (the runtime's transcript
  store) for which allowlisted patterns were actually exercised.
