...
- Verification: what command, test, or observable behavior proves each
  requirement?
- Rigor: prototype or production? Ask this as one choice — prototype-grade
  work legitimately skips verification that production-grade work must pay
  for, and the failure mode is not declaring a point on that spectrum. On
  "prototype", step 4 writes a `Rigor: prototype` header; on "production" or
  no answer, write nothing (absent means production).
...
placement convention as `Priority:`. On production or no answer, write
nothing: absent means production (the only legal values are `prototype` and
`production`).
