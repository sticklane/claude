...
**Invocation gate.** The read-only report mode is safe to auto-trigger — same
risk class as `/critique` and `/simplify`. **`--fix` is human-typed only:**
never infer or add it from a vague request ("clean up this doc" is a review
...
3. **Reader test — for orientation docs (README.md, AGENTS.md) by default.**
   Open one fresh Agent Manager conversation per reference.md's "reader test"
   procedure (session tier, awaited, capped return) and merge its stumble
   report. Skip it for diffs and pasted text — a fragment has no cold-read
   context to test.
