...
Three tics the nine-item rubric does not name. They live in a register the
rubric's charter — human-facing docs — does not reach: agent-authored status
and progress reports (workflow output, task summaries). They are documented as
general RLHF-trained assistant behaviors, not tied to one model, so they bind
every runtime this doctrine ports to equally; much of the evidence is measured
on GPT, so they apply at least as strongly to Gemini and GPT agents as to any
other. Each tell states a name, a one-line description, a cited source, a
before→after rewrite, and a boundary against the rubric item nearest it. Any
count cited is measured on one operator's corpus and is illustrative of the
general pattern, not a per-runtime figure.

**T1 — meta-discourse (announce-then-do process narration).** A clause spent on
what the answer is *about to* do, or on the response's own structure, instead
of the substance. "Let me check the config", "Now I'll run the tests", "In this
section I explain…" — the reader already sees the action happen. Source:
...
preamble is exactly the length RLHF rewards.
...
- Boundary vs item 6 (stock acknowledgments): item 6 is a contentless
  pleasantry ("Got it"); T1 previews *work*, naming an action about to fire.
...
measured timing — inventing fuzz over a fact the writer can state exactly.
...
- Boundary vs item 2 (hedging/disclaimers): item 2 reads *less* committal than
  warranted; T2 reads *more* precise than warranted over a known number.

**T3 — evaluative varnish substituting for the checkable fact.** A quality
adjective — "clean", "solid", "robust", "holistic" — used in place of the fact
it summarizes. "Landed clean" names no conflict status or gate result; "clean
merge" names no test outcome. Intensifiers ("genuinely") and salience labels
("notably", "clearly") inflate the same way. Source: Liang et al., "Monitoring
AI-Modified Content at Scale" (ICML 2024, arXiv:2403.07183) — adjectives are the
most discriminative AI-overused part of speech ("robust", "holistic" in the
Top-100 table); Kobak et al. (arXiv:2406.07016) find style words dominate excess
vocabulary.
...
report a checkable fact tersely — exempt, and never to be padded into prose.
