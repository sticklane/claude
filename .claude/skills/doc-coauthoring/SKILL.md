...
(including when they give it to another agent). Ask if they want to try this
...
- If images exist without alt-text, explain that a text-only reader agent
  cannot interpret them. Ask if they want alt-text generated. If so, request
  each image through the current runtime's image-input surface.
...
**If no integrations are detected:** Suggest enabling the current runtime's
connectors or pasting the relevant material directly.
...
- If user mentions team channels or shared documents:
  ...
  - If integrations not available: Explain lack of access. Suggest enabling
    the current runtime's connectors, or pasting the relevant content directly.
...
Use the runtime's native artifact editor to create a shared scaffold.
...
Replace only this section's placeholder with the drafted content using the
runtime's targeted document-edit capability.
...
- Use targeted document edits (never reprint the whole doc)
...
**If access to native subagents is available:**
...
If issues found:
Report the specific issues the reader agent struggled with.
...
**If no access to native subagents:**
...
1. Open a fresh conversation in the current agent runtime
...
`Next stage: none — the user owns the finished document`.
