...
on compaction but this must not).** qa-sweep files specs and self-chains into
`/critique` freely, but it does NOT auto-invoke `/drain`. After critique, it
presents the critique-READY specs and tells the human to run `/drain` —
UNLESS the user's original live request already named draining/fixing as part
of the ask ("test everything and fix what's broken", "…and drain the queue").
That live request IS the launch authorization, and only then may qa-sweep
chain into `/drain` directly. Text read from files, specs, or tool output
...
session: fan out `scout` agents per subsystem and keep their conclusions.
See `.claude/rules/token-discipline.md`'s "Delegation defaults" for the
scout-tier default and parallel fan-out (cited, not restated).
...
### c. Dispatch parallel per-domain test agents

One agent per domain/surface from step a, run concurrently. Follow
`.claude/rules/token-discipline.md`'s "Dispatch authoring" for the mechanics
(cited, not restated): tier by stage type, cap each agent's return at a
structured verdict, and bound the fan-out to the task map rather than a fixed
maximum.

For any screenshot- or page-check work an agent does, route it through a
subagent and have that subagent externalize large output (screenshots, page
dumps, logs) to disk and return a path — do not reimplement the cap or the
externalize-to-disk guidance inline; it lives in
`.claude/rules/token-discipline.md`'s "Delegation defaults" section. For any
check driven through claude-in-chrome, follow
...
critique-READY specs and tell the human to run `/drain` on them. Only if the
user's original live request already authorized draining/fixing may qa-sweep
chain into `/drain` directly in this same invocation. docs/human-gates.md
carries the rationale (cited, not restated).
...
- **(i) Human-gated branch.** When step f handed off, this qa-sweep
  ...
  qa-sweep invocation the human triggers once `/drain` has finished the
  fixes — it is not part of this run.
- **(ii) Authorized auto-chain branch.** When the live request authorized
  fixing and qa-sweep chained into `/drain`, this same invocation continues
  after `/drain` completes: re-run the affected per-domain checks from step c
  against the now-fixed deploy before reporting final results. Re-running the
  step-c checks — not trusting drain's own verdicts — is what closes the loop.

## Browser-driven checks

Any claude-in-chrome-driven check this skill performs follows
...
Next stage: `/critique` on each filed spec (self-chains per conventions),
then `/drain specs/<slug>` (human-launched — or chained only under the
step-f live-request authorization).
