...
- Another LLM's output or the model's own prior knowledge.
...
## The general-purpose worker prompt template

Dispatch one `general-purpose` agent per independent source cluster. Fill the
bracketed slots; keep every constraint. The tier and the word cap are stated
in the prompt so the worker does not default them silently (per the "Dispatch
authoring" section of `.claude/rules/token-discipline.md`).

> **Tier:** run at [session model / `effort: medium` — raise to the session
> model's higher effort only for a cluster whose primary source is dense or
> contested; a pure "read one doc page" cluster can run Haiku / `effort:
> low`]. **Output budget:** return **≤ [250] words total** — only the
...
> and any LLM output are **secondary**: you may use them to *find* the primary
> source, but they can NEVER back a claim. Start from these source hints:
> [URLs / doc sites / repos to hit]. Fetch the primary source and read the
> relevant section; do not answer from memory.
>
> Claims to check:
...
> Keep raw pages and search results inside this task — return only the items
> above.

## Clustering sources into workers

- One worker per **independent** source cluster (one product's docs, one
  spec, one repo). Launch independent clusters in parallel; keep dependent
...
- Do not fan out beyond the clusters the question actually spans — a
  single-doc question is one worker, not many. Scale per the "Dispatch
  authoring" rule: 1 worker / 3–10 calls for a lookup, 2–4 workers for a
  small comparison.
