---
name: factcheck
description: Closes a known-source factual gap with cited primary sources — dispatches targeted general-purpose web workers, each backing every claim with a verbatim quote + URL or marking it UNVERIFIED. Use to verify a claim ("fact-check this", "is it true that…", "cite the source for…", "verify X against the docs") or to look up a known source ("what do the official docs say about…", "get me the primary source for…", "close this factual gap"). NOT for open-ended, multi-source synthesis where the claims are contestable — that stays deep-research.
...
## 2. Dispatch one general-purpose worker per source cluster

The workers must be `general-purpose` agents — they have WebSearch/WebFetch;
the toolkit's scout/verifier/critic do not. Launch one per **independent**
source cluster, in parallel when the clusters don't depend on each other.

Each worker prompt (exact template in `reference.md`, filled per cluster)
states, so nothing defaults silently:
- an explicit **tier** — session model at `effort: medium` for a dense or
  contested cluster, Haiku / `effort: low` for a pure single-doc lookup;
...
Raw fetched pages stay inside the worker; only the cited findings return to
you. Do not fan out beyond the clusters the question spans — a single-doc
question is one worker (see `reference.md` "Clustering sources into workers").
...
this is the wrong tool — that is `deep-research`. This skill is for closing a
*known-source* gap, per the "Match the research tool to the question" section
of `.claude/rules/token-discipline.md`.
