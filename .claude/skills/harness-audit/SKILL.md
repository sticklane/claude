...
are BOTH read-only AND permitted by the repo's allowlist, and inspects
(never executes) mutating commands. Every finding's next step is a normal
task/spec filed through the pipeline — never an in-audit edit. `git status`
...
cross-checks). Dispatch each to a scout-tier agent per the dispatch-tier
rule below; only the final ranking/synthesis step (area 6) runs on the
session model.

### 1. Command currency

Scope every documented command (from `AGENTS.md`'s `## Commands`, CLAUDE.md,
build files) by mutation class BEFORE running anything:

- **Execute** a command only if it is BOTH read-only AND permitted by the
  repo's permission allowlist. Allowlist membership alone is NOT sufficient —
  an allowlist can contain mutating commands (e.g. `Bash(git commit:*)`), and
  executing one would break R1's read-only contract. An executed command must
  exit 0 (or match a documented expected failure); a nonzero exit is a
  finding.
...
the ranking/synthesis step (area 6) runs on the session model — it is the
judgment stage (R4).
