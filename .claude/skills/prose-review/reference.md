...
Procedure: spawn ONE fresh-context agent (session tier, no prior context —
it must read cold, exactly as a first-time reader would; per
`.claude/rules/token-discipline.md` Dispatch authoring — awaited child,
capped return). Give it only the target document. It reads once and answers,
in a capped report:
