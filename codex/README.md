...
- Only the three launch-gated stages — `drain`, `build`, and
  ...
  drain/build since 2026-07; the `disable-model-invocation` flag
...
reused symlinked skills and the three new ones. No root-level
...
[#23454](https://github.com/openai/codex/issues/23454). The three stages still
...
- **Explicit-invocation-only skills are experimental.** The three
  human-launched stages (`drain`/`build`/`evals`) rely on
  `allow_implicit_invocation: false` plus `$name` / `/skills` invocation.
  Live-tested here: the _guard_ works (no auto-trigger — (c-neg) above), but
  the _explicit `$name` path is not yet exposed in `codex exec`_ — (c-pos)
  above, matching upstream #19695/#10585/#23454. Treat these three as
...
- **Subagents are not a workflow launcher.** Codex Subagents (`.codex/agents/`
  ...
  command. They do not provide a launch-gate stand-in for the three stages.
- **The launch guard is real, but the file is still on disk.** Because
  `allow_implicit_invocation: false` only removes a skill from _auto-selection_
  ...
  `verify-live.sh` does) so a stage that is merely _read_ still cannot act.
...
| `qa-sweep`        | Not ported — qa-sweep is a skill, not one of the three explicit-invocation-only skill wrappers (drain/build/evals) Codex ships as real content, so per root CLAUDE.md's mirror-chain convention no `codex/.agents/skills/qa-sweep` entry is created.                                                                                                             |
| `work`           | Not ported — added after the 2026-07-22 data-portability pivot (mirror content superseded, queued for deletion); Codex reads the bd data layer (`bd list --json` / committed `.beads/issues.jsonl`) directly.                                                                                                                                                |
...
`antigravity/.agents/skills/*` directories, adds only the three
explicit-invocation-only skill wrappers as real content). A change to one of
`.claude/skills/{drain,build,evals}/SKILL.md` must also update the
