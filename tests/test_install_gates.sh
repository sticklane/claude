...
# ---------------------------------------------------------------------------
# R7a legacy install-gates dead-hook repair: known terminal exit 0 hooks are
# regenerated in place (archive skipped) so chained managed blocks are not dead.
# ---------------------------------------------------------------------------

ST="$TMP/staleinstall"
mkrepo "$ST"
printf 'requests\n' > "$ST/requirements.txt"
cat > "$ST/.git/hooks/pre-commit" <<'EOF'
#!/usr/bin/env bash
...
exit 0
EOF
chmod 755 "$ST/.git/hooks/pre-commit"

run_install "$ST"
assert_eq "R7a stale: exits 0" 0 "$RI_EXIT"
assert_not "R7a stale: no pre-commit archive created" \
  test -e "$ST/.git/hooks/pre-commit.pre-gates"
assert "R7a stale: stale hook repaired with managed template" \
  grep -q 'Generated by install-gates' "$ST/.git/hooks/pre-commit"
assert_not "R7a stale: stale hook repaired to non-terminal exit 0" \
  grep -q '^exit 0$' "$ST/.git/hooks/pre-commit"

ST1="$(managed_hash "$ST")"
run_install "$ST"
ST2="$(managed_hash "$ST")"
assert_eq "R7a stale: second install is idempotent" "$ST1" "$ST2"
