...
- [x] `grep -c "checked: evals" tests/mirror-procedure-manifest.txt` → ≥1, OR a new manifest line referencing `evals` — evidence either way (grep → 1; also seeded a phrase line `.claude/skills/evals/SKILL.md|antigravity/.agents/workflows/evals.md|skill-authoring gap`) — verifier PASS (2026-07-16 sweep)
- [x] `for t in tests/test_*.sh; do bash "$t" || echo "FAIL: $t"; done` → no FAIL lines (all suites green, 0 FAIL lines) — verifier PASS (2026-07-16 sweep)
