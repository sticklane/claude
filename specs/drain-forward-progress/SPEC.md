...
- R2 **Screen matches instructions, not mentions.** `screen-stub.sh`'s
  ...
  directive), not on any occurrence. The three 2026-07-11 stub bodies
  become regression fixtures: stub 08's descriptive "$HOME data" text
  must pass; a seeded "run rm -rf /Users/x/…" fixture must still refuse.
  Screen behavior stays deterministic (regex list + fixtures in the
  script's test).
...
- [ ] `bash .claude/skills/drain/screen-stub.sh specs/agentprof-attribution-gaps/tasks/08-pending-match-meta-sidechain-investigation.md`
  ...
  data" text that false-positived today) AND a seeded
  instruction-shaped fixture still refuses (script test or fixture pair
  committed beside the script) (R2)
