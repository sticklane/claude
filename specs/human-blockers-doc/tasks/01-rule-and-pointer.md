...
- [x] `test -f .claude/rules/human-blockers.md && grep -qi 'Agent-filed blockers' .claude/rules/human-blockers.md` → hits (anchor 0-hit everywhere before this spec) — verifier PASS, exit 0 (evidence/01-rule-and-pointer.md)
- [x] `grep -qi 'human-blockers' CLAUDE.md` → hit (0 today, verified) — verifier PASS, match at CLAUDE.md:123 (evidence/01-rule-and-pointer.md)
- [x] `wc -l < .claude/rules/human-blockers.md` → < 60 — verifier PASS, 40 lines (evidence/01-rule-and-pointer.md)
