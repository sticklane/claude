...
- [ ] `python3 -m pytest .claude/skills/workboard/test_workboard.py -q` → pass incl. the fixture pair (two open + one checked → two rows; no HUMAN.md → no rows, no error)
- [ ] `diff -q .claude/skills/workboard/workboard.py antigravity/.agents/skills/workboard/workboard.py && diff -q .claude/skills/workboard/test_workboard.py antigravity/.agents/skills/workboard/test_workboard.py` → identical
- [ ] `bash agent-console/scripts/check.sh` → green (adapter untouched or pass-through only)
