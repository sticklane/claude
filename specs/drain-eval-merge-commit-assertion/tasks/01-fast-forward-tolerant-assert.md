...
- [ ] Fixture A (task 01 fast-forwards, task 02 real merge) → `assert.sh` passes; all three checks correctly recognize task 01's landing
- [ ] Fixture B (both tasks real merges) → `assert.sh` passes (no regression)
- [ ] Fixture C (task 01 fast-forwards; its done-flip commit itself also touches a file outside its Touch list) → `assert.sh` fails, naming the Touch violation
- [ ] Fixture C' (task 01 fast-forwards across TWO commits: an earlier commit with an out-of-Touch edit, then a separate later done-flip-only commit) → `assert.sh` fails, naming the Touch violation from the earlier commit — proves the Touch check diffs the full landed range, not just the done-flip commit
- [ ] Fixture D (one fast-forward commit flips two task files' Status at once — a barrier, not a rolling-window landing) → `assert.sh` fails the barrier check
- [ ] Fixture E (both tasks fast-forward, zero merge commits anywhere in history) → `assert.sh` passes — proves no hidden merge-count floor remains
- [ ] `bash evals/drain/01-rolling-window/assert.sh` run against all 6 fixtures above produces the documented pass/fail in a single session's log (paste or summarize each run's actual output in the evidence file)
- [ ] `specs/drain-rolling-window/evidence/06-drain-eval-scenario.md` updated per step 5
- [ ] Optional: one full `bash evals/run.sh drain` end-to-end run passes (not required to reproduce the fast-forward case specifically)
