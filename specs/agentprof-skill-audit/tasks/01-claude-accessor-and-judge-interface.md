...
Priority: P0
Budget: 28 turns
Spec: ../SPEC.md (requirements R2, R4, R5, R6, R9, R12)
Touch: agentprof/internal/claude/skill_invocations.go, agentprof/internal/claude/skill_invocations_test.go, agentprof/internal/claude/skill_frontmatter.go, agentprof/internal/claude/skill_frontmatter_test.go, agentprof/internal/judge/judge.go, agentprof/internal/judge/cli.go, agentprof/internal/judge/cli_test.go, agentprof/internal/judge/fake.go

## Goal

Three foundational, independently-testable pieces every other task in this
spec builds on exist and are unit-tested — deliberately consolidated here
(not split into 02/03) because both 02 and 03 need SKILL.md frontmatter
parsing and putting it in two concurrent `package main` files would
duplicate a helper under the same namespace and break the build at merge
time: (1) a new exported `SkillInvocations` accessor on
`agentprof/internal/claude` that surfaces what `internal/claude`'s
unexported internals already parse but don't expose — raw `Skill`
`tool_use` blocks paired with their `tool_result`, each invocation's
preceding `<command-name>` tag (if any), and whether an intervening user
turn preceded it; (2) a new exported `SkillFrontmatter(path) →
{Description, OutcomeRubric, DisableModelInvocation}` accessor on the same
package that parses a `SKILL.md`'s frontmatter fields — the SINGLE shared
parser task 02 (description, disable-model-invocation) and task 03
(outcome-rubric) both call, so neither writes its own; (3) a `judge`
package (new, `agentprof/internal/judge/`) providing a `Judge` interface
mirroring `agentprof/internal/naming/cli.go`'s `Namer`/`CLINamer` shape, a
real CLI-backed implementation (`claude -p <prompt> --model <tier>
...
4. Write a failing test in `skill_frontmatter_test.go` first: fixture
   SKILL.md files covering `description:` present, `outcome-rubric:`
   present, `disable-model-invocation: true` present, and a minimal
   SKILL.md with none of the optional fields. Implement
   `SkillFrontmatter` to pass.
5. Read `agentprof/internal/naming/cli.go` and `cli_test.go` for the
   subprocess-invocation and command-construction-test patterns to mirror.
6. Write a failing test in `judge/cli_test.go` first, mirroring
...
7. Implement the `Judge` interface, `CLINamer`-equivalent CLI
