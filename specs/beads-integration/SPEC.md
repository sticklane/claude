...
> **Closed 2026-07-03 — both tasks resolved.** `~/specs/work-tracking-consolidation/`
...
> full exit — hybrid retired").
> - **Task 02 (optional beads queue backend): withdrawn** — its premise
>   (industrialize the drain queue layer with beads) is contradicted by the exit.
> - **Task 01 (discovered-work capture): re-homed** to
>   `specs/discovered-work-capture/` (zero-dependency, independent of beads —
>   the `discovered-from` concept still earns its place). Dispatch it from there;
>   this spec is retained only as a tombstone for the decision record below.
