...
- [x] `grep -qi 'Diátaxis' antigravity/.agents/skills/prose-review/reference.md` (or SKILL.md) → hit — antigravity mirror reference.md carries the Diátaxis quadrant table; grep HIT.
- [x] `claude plugin validate .` → passes — "✔ Validation passed".
- [x] `git show HEAD -- .claude-plugin/plugin.json | grep -q '^+.*"version"'` run right after this task's commit → hit — version bumped 0.8.49 → 0.8.50 in this task's commit.
- [x] `grep -c 'automation\|dev-agents' specs/prose-review/evidence/zero-target.md` ≥ 2 — count = 2 (one entry each for ~/automation and ~/dev-agents).
