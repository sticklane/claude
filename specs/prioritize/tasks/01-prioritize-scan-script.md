...
- [x] `python3 -m pytest .claude/skills/workboard/ .claude/skills/list-specs/ -q` (the real per-subproject check; repo has no top-level `scripts/check.sh`, `ls scripts/` → absent) → green — `87 passed` (importlib reuse intact), plus the new suite `13 passed` (evidence/01-prioritize-scan-script.md)
