...
2. Rewrite `live_session_ids()` per SPEC.md Solution A: CLI primary, PID-scan fallback, 2-tuple return. Session *rows* keep rendering from the `.jsonl` transcript via `_first_prompt_and_meta`; CLI `cwd`/`name` are NOT used for row content.
...
- [ ] `python3 /Users/sjaconette/claude/.claude/skills/workboard/workboard.py --out /tmp/wb-task01.html` → exits 0 and the HTML marks sessions active consistent with `claude agents --json` (diff the active sids)
